...
- Added concise tool-use activity to Commentary, with distinct subagent, runbook, memory, command, and fallback icons, present-progressive tool-specific labels, paired-result deduplication, plural summaries for consecutive repeated calls, nested disclosures for each call's input and output, and automatic expansion while a tool is the latest chat item.

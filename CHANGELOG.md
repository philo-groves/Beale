...
- The workspace dashboard now uses Overview, Activity, and Resources tabs. Overview can edit the workspace name, description, and scope rules while presenting directory, subject, and research profile as disabled context; Activity contains the session Gantt chart; and Resources retains resource-type tabs, tiled lists, and editing while the right sidenav remains persistent across views.
...
- Agent Settings now presents research profiles in a two-level Profiles view. Each profile defaults to an Overview tab with centered Basic Details and Heat forms; Heat shows one palette variant at a time, defaults to Dark, and pairs generic level guidance with right-aligned color controls. Aligned memory-type tabs sit directly beneath the profile tabs and open full-width definition and session-heat panels.

...
- The workspace dashboard now uses Overview, Activity, Resources, Memory, Runbooks, and Clean tabs in a centered full-width layout. Overview persists editable workspace metadata in place when a field loses focus; Memory and Runbooks reuse their detailed catalog rows and open the right detail panel only after a selection, while Clean presents Settings-style Dejunk and Dream actions with live counts and progress states.
- Workspace Activity, Resources, Memory, and Runbooks now share a centered, flat yearly token-usage heatmap; the Activity Gantt and Memory and Runbook catalogs remain full-width beneath it.

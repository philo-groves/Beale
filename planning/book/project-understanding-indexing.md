...
- Search assembly now normalizes lexical, artifact, metadata, semantic, graph, and graph-variant hits into a common internal retrieval candidate envelope with entity identity, source path, range, namespace, score, signals, and provenance before final tool output formatting.
- Search retrieval now runs in two phases. Phase A gathers broad lexical, artifact, metadata, semantic, graph-proximity, and graph-variant candidates into the normalized envelope; Phase B expands only the best seeds through containing source structures, binary inventory anchors, graph neighborhoods, and graph variants.
- Search scoring now blends explicit signal components for text relevance, exact identifier/path matches, structural fit, semantic similarity, graph proximity, evidence/research-memory linkage, security relevance, scope confidence, recency, and provenance instead of ranking primarily by source kind.
- Search dedupe now merges candidates that share an entity or precise file/range. Merged results preserve contributing-source provenance for direct lexical, semantic, graph-adjacent, and graph-variant hits, and prefer richer entity-backed records over plain file hits when they cover the same location.
- Search planning now adds lightweight query intent hints for symbol lookup, route/API lookup, auth/permission questions, sink/data-flow questions, binary orientation, prior research-memory lookup, and variant/similarity search. Intent hints weight retrieval scoring but do not exclude any layer.
- Search result selection now preserves the top exact result, then diversifies by source path and relationship family so graph/semantic context can pull in related files instead of returning a same-file wall of chunks. Per-source caps relax when the query looks like a single file or symbol lookup.
- Search payloads now include retrieval diagnostics with candidate counts by layer, selected counts by layer, dedupe count, graph expansion count, top scoring signals, and selected relationship-family counts so indexing gaps can be distinguished from ranking decisions.
- Retrieval diagnostics now include explicit missing reasons for stale or empty semantic/graph indexes, graph-ready searches with no expansion seeds, semantic candidates that ranking did not select, and query-intent/index mismatches such as route/API intent without indexed routes or route graph edges.
- Search now performs one bounded adaptive follow-up query when the first pass has weak coverage, such as no semantic hits, no graph hits, or same-file lexical-only results. The follow-up uses query intent hints to add related metadata, semantic, graph, and variant candidates without excluding the original results.
- Search now annotates the top selected matches with read-budget hints: `readPriority`, `readReason`, and `suggestedNextRead` code-browser arguments so the model can spend follow-up reads on the highest-value context first.
- Search scoring now applies a uniform negative/low-confidence penalty across metadata, semantic, and graph-backed research-memory candidates for duplicate, dismissed, false-positive, not-reproducible, out-of-scope, and low-evidence states while keeping those records retrievable.
- Search scoring now applies bounded cross-run learning feedback from accepted `code_browser` reads, verifier-backed hypotheses/findings, and corrected negative research states. Diagnostics expose feedback counts so repeated misses can be separated from absent learning signals.
- Search diagnostics now include concise layer-specific operational hints for source materialization, indexing, semantic freshness, graph readiness, missing route/permission/sink edges, binary coverage, and research-memory coverage.

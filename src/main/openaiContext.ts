...
export function buildOpenAiInstructions(scope: ProgramScopeVersion, input: StartRunInput): string {
  ...
  return [
    ...
    'Use `finding` when evidence suggests a durable issue. Only mark a finding verified after a real passing verifier run. Only mark it reportable when verified behavior, attacker reachability, and exploit practicality are all certain enough for disclosure review; otherwise leave it reproduced, suspected, dismissed, or out_of_scope as appropriate.',

...
Auto-Review receives only compact host facts about recorded authorization and operator-managed execution posture. If it denies an otherwise reviewable command, the affected session replaces its steering composer with a correlated Approve Once question; the researcher can approve only that exact command or keep it blocked. Policy failures and unavailable or malformed reviews remain fail-closed.

...
For active runs, Honeycrisp hosts a versioned WebSocket session endpoint bound to loopback. Beale authenticates with a per-process bearer token kept in the child environment, then exchanges client-neutral session event and control envelopes. Honeycrisp owns this protocol; Beale only adapts it into desktop state. There is no Beale-specific stdio event or control protocol.

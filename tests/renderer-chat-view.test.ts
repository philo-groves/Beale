...
describe('renderer Chat View preference', () => {
  ...
  it('renders Session View and Permissions forms in General settings', () => {
    ...
    const generalFormStyles = styles.match(/\.general-settings-page \.settings-form,\s*\.profile-settings-page \.settings-form\s*\{([^}]*)\}/)?.[1] ?? '';
    const formHeadingStyles = styles.match(/\.settings-form-heading\s*\{([^}]*)\}/)?.[1] ?? '';
    const headingStyles = styles.match(/^\.settings-form-heading h2\s*\{([^}]*)\}/m)?.[1] ?? '';
    ...
    expect(pageStyles).toContain('--settings-view-font-size: 14px');
    expect(pageStyles).toContain('font-size: var(--settings-view-font-size)');
    ...
    expect(generalFormStyles).toContain('font-size: var(--settings-view-font-size)');
    const generalControlStyles = styles.match(/\.general-settings-page \.settings-form-control-row,([\s\S]*?)\{([^}]*)\}/)?.[2] ?? '';
    const nonProviderHeadingStyles = styles.match(/\.general-settings-page \.settings-form-heading h2,([\s\S]*?)\{([^}]*)\}/)?.[2] ?? '';
    expect(generalControlStyles).toContain('font-size: var(--settings-view-font-size)');
    expect(styles).toContain('.general-settings-page .chat-view-option,');
    expect(nonProviderHeadingStyles).toContain('font-size: 26px');

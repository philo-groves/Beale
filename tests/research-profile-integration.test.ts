...
describe('research profile host integration', () => {
  ...
  it('applies a non-security profile to recommendations and context without expanding host tool authority', async () => {
    ...
    try {
      ...
      expect(providerMismatchArgs[providerMismatchArgs.indexOf('--title-model-default') + 1]).toBe('grok-4.3');
      expect(providerMismatchArgs[providerMismatchArgs.indexOf('--title-effort-default') + 1]).toBe('medium');
      expect(JSON.parse(
...
function fakeHoneycrispProtocolSource(): string {
  return [
    ...
    "const value = (flag) => args[args.indexOf(flag) + 1];",
    "const success = (operation, result) => ({ protocol: 'honeycrisp', protocolVersion: 1, operation, requestId: value('--request-id'), ok: true, result });",
    "const input = JSON.parse(readFileSync(value('--input'), 'utf8'));",
    "if (args[0] === 'harness' && args[1] === 'model-job-resolve') { const job = !input.job?.provider || input.job.provider === input.provider || (input.provider === 'openai-codex' && input.job.provider === 'openai') ? input.job : null; const result = { provider: input.provider, model: input.requestedModel || job?.model || input.configuredModel || input.fallbackModel, effort: input.requestedEffort || job?.effort || input.configuredEffort || input.fallbackEffort }; console.log(JSON.stringify(success('model_job.resolve', result))); process.exit(0); }",
    "if (args[0] === 'harness' && args[1] === 'provider-describe') { const result = { providers: ['openai-codex', 'anthropic', 'xai', 'zai', 'openrouter'], aliases: { openai: 'openai-codex' }, defaultSmallModels: { 'openai-codex': 'gpt-5.6-luna', anthropic: 'claude-haiku-4-5', xai: 'grok-4.3', zai: 'glm-5-turbo', openrouter: 'auto' }, auxiliaryEfforts: ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'], sessionTitleEffort: 'medium', shellReviewEffort: 'medium' }; console.log(JSON.stringify(success('provider.describe', result))); process.exit(0); }",
    "if (args[0] === 'harness' && args[1] === 'maintenance-summary') { const result = { available: true, newFileCount: 0, newFileCountCapped: false, baselineAt: new Date().toISOString(), lastRun: null }; console.log(JSON.stringify(success('maintenance.summary', result))); process.exit(0); }",
    "if (args[0] === 'harness' && args[1] === 'source-inspect') { const result = { urls: [], normalizedUrl: null, candidates: [] }; console.log(JSON.stringify(success('source.inspect', result))); process.exit(0); }",
    "if (args[0] === 'harness' && args[1] === 'plugin-list') { const result = { registryPath: '/plugins.json', pluginStorePath: '/plugins', specVersion: '1.0.0', plugins: [{ id: 'beale-introspection-builtin', name: 'beale-introspection', version: '0.1.0', description: 'fixture', enabled: true, status: 'ready', source: { kind: 'builtin', path: '/beale-introspection' }, installedAt: new Date().toISOString(), updatedAt: new Date().toISOString(), skills: [], mcpServers: [{ name: 'beale', transport: 'stdio', command: 'node', url: null, valid: true, errors: [] }], warnings: [], errors: [] }] }; console.log(JSON.stringify(success('plugin.list', result))); process.exit(0); }",
    "if (args[0] === 'harness' && args[1] === 'plugin-runtime') { const result = { runtimeDirectory: '/plugin-runtime', skillDirs: [], selectedSkillIds: [], mcpConfigPath: '/plugin-runtime/mcp.json', allowedMcpServers: ['beale-introspection.beale'], args: ['--mcp-config', '/plugin-runtime/mcp.json', '--allow-mcp-server', 'beale-introspection.beale'], warnings: [] }; console.log(JSON.stringify(success('plugin.runtime', result))); process.exit(0); }",
    ...
    "const campaign = { nodes: [], edges: [], coverageGaps: [], contradictions: [], momentum: { state: 'empty', reason: 'No campaign work yet.', supportingNodeIds: [] }, nextActions: [], counts: { findings: 0, verifiedFindings: 0, disclosedFindings: 0, coverageGaps: 0, contradictions: 0 } };",
    "const result = { status: 'empty', source: 'honeycrisp_sqlite', contextWorkspaceId: input.workspaceId, contextSubjectId: input.subjectId, activeCatalogHash: input.researchProfile?.profileHash ?? null, databasePath, storageRoot: dirname(databasePath), artifactDirectoryPath, databaseSizeBytes: 0, nodeCount: 0, edgeCount: 0, evidenceRefCount: 0, storageArtifactCount: 0, runbookCount: 0, reportCount: 0, latestNodeUpdatedAt: null, nodeTypeCounts: {}, nodeStatusCounts: {}, nodeProvenanceCounts: {}, nodes: [], edges: [], runbooks: [], reports: [], findings: [], campaign, dreaming: { available: false, scope: 'workspace', hiddenNodeCount: 0, restorableChangeCount: 0, lastRun: null, changes: [] }, directories: [{ name: 'artifacts', path: artifactDirectoryPath, purpose: 'Honeycrisp artifact storage.', exists: false, entryCount: 0 }], lastError: null };",
    "console.log(JSON.stringify(success('memory.summary', result)));"
  ].join('\n');
}

...
describe('benchmark and calibration runner', () => {
  ...
  it('records Docker benchmark results and compares same-model different-harness runs on a stable sample', async () => {
    ...
    const failedTaskResult = candidate.benchmark.latestResults.find((result) => result.taskId === 'smoke-artifact-capture');
    const firstTaskResult = candidate.benchmark.latestResults[0];
    expect(failedTaskResult?.status).toBe('inconclusive');
    ...
    expect(firstTaskResult?.metrics.modelProxyCalled).toBe(true);
    expect(firstTaskResult?.metrics.modelProxyRequests).toBeGreaterThan(0);
    expect(firstTaskResult?.metrics.sessionTokenCount).toBeGreaterThan(0);
    expect(firstTaskResult?.metrics.sessionDurationMs).toBeGreaterThanOrEqual(0);
    expect(firstTaskResult?.metrics.turnCount).toBeGreaterThan(0);
    expect(firstTaskResult?.metrics.timeToFindingMs).toBeGreaterThan(0);
    expect(failedTaskResult?.graderReport.failReason).toBe('Missing required evidence: artifact');
    expect(candidate.benchmark.recentResults.some((result) => result.id === failedTaskResult?.id)).toBe(true);
